 ...
 // パンズームバーの表示(パン：十字キー ズームバー：拡大縮小)
    ...
    $(function() {
    	$.ajax({
    		type : "POST",
    		url : "http://160.16.233.165/rest_score/Restaurants/getRestData",
    		data : {
    			"data" : ""
    		},
    		dataType : "json",
    		success : function(data) {
    		    ...
    		    for (var i=0;i<data["length"];i++) {
    		        ...
    		        var marker = new OpenLayers.Marker(
        		        ...
        		        new OpenLayers.Icon("http://160.16.233.165/img/"+data[i][1],iconsize,point)
        		        );
        		    marker.tag = "店名:"+data[i][0];
        		    marker.x = x;
        		    marker.y = y;
    ...
    function successCallback(position) {
      ...
      //マーカーの追加
        ...
        //ログイン中のアカウントの画像を取得とDBの現在位置を更新
        $(function() {
        	$.ajax({
        		type : "POST",
        		url : "http://160.16.233.165/rest_score/Webaccounts/getImage",
        		data : {
        			"x" : x,
        			"y" : y
        		},
        		...
        		success : function(data) {
        			var marker = new OpenLayers.Marker(
        		            new OpenLayers.LonLat(x,y).transform(projection4326,projection3857),
        		            new OpenLayers.Icon("http://160.16.233.165/img/"+data[0],iconsize,point)
        		    );
